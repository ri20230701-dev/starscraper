...
| 街区内の最終 push | **歩行者数** | 30日以内の活動量は1、365日以上・日時なしは0、その間はlog空間で補間。街区を囲む道路を周回する。活動量1につき4人、街区24人・全体240人まで。全体上限で省いた人数はDTOの `omittedPedestrians` に記録 |
| 最終 push | **店のシャッターの開き具合** | 歩行者と同じ活動量で開く。365日放置で完全閉鎖。店は最下段の窓1行を置き換える |
| fork された数 | **店の客入り（ショーウィンドウの明るさ）** | fork数をlog2で反映し、4096で最大。シャッターが閉じた店の光は見えない |
| fork | 低層・彩度低め | 形は変えない。高さと窓が意味を運ぶのを邪魔しないため |
| 道路グリッド | **街灯** | リポジトリの意味を運ばないインフラ。道路セルに等間隔で配置し、暖色が多い窓と対比する寒色寄りの白を使う |
